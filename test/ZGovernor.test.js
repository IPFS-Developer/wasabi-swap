...
contract('Governor', ([alice, minter, dev]) => {
    it('should work', async () => {
        ...
        await this.wasabi.delegate(dev, { from: dev });
        this.chef = await MasterChef.new(this.wasabi.address, dev, '100', '0', '0', { from: alice });
        ...
        // Perform another deposit to make sure some WASABIs are minted in that 1 block.
        await this.chef.deposit(0, '100', { from: minter });
        assert.equal((await this.wasabi.totalSupply()).valueOf(), '110');
        assert.equal((await this.wasabi.balanceOf(minter)).valueOf(), '100');
        assert.equal((await this.wasabi.balanceOf(dev)).valueOf(), '10');
        // Transfer ownership to timelock contract
        this.timelock = await Timelock.new(alice, time.duration.days(2), { from: alice });
        this.gov = await GovernorAlpha.new(this.timelock.address, this.wasabi.address, alice, { from: alice });
