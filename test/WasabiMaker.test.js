...
const WasabiMaker = artifacts.require('WasabiMaker');
const MockERC20 = artifacts.require('MockERC20');
const UniswapV2Pair = artifacts.require('UniswapV2Pair');
const UniswapV2Factory = artifacts.require('UniswapV2Factory');

contract('WasabiMaker', ([alice, bar, minter]) => {
    beforeEach(async () => {
        this.factory = await UniswapV2Factory.new(alice, { from: alice });
        this.wasabi = await WasabiToken.new({ from: alice });
        await this.wasabi.mint(minter, '100000000', { from: alice });
        ...
        this.maker = await WasabiMaker.new(this.factory.address, bar, this.wasabi.address, this.weth.address);
        this.wasabiWETH = await UniswapV2Pair.at((await this.factory.createPair(this.weth.address, this.wasabi.address)).logs[0].args.pair);
    ...
    it('should make WASABIs successfully', async () => {
        await this.factory.setFeeTo(this.maker.address, { from: alice });
        await this.weth.transfer(this.wasabiWETH.address, '10000000', { from: minter });
        await this.wasabi.transfer(this.wasabiWETH.address, '10000000', { from: minter });
        await this.wasabiWETH.mint(minter);
        ...
        // After calling convert, bar should have WASABI value at ~1/6 of revenue
        await this.maker.convert(this.token1.address, this.token2.address);
        assert.equal((await this.wasabi.balanceOf(bar)).valueOf(), '32965');
        assert.equal((await this.token1Token2.balanceOf(this.maker.address)).valueOf(), '0');
        // Should also work for WASABI-ETH pair
        await this.wasabi.transfer(this.wasabiWETH.address, '100000', { from: minter });
        await this.weth.transfer(this.wasabiWETH.address, '100000', { from: minter });
        await this.wasabiWETH.sync();
        await this.wasabi.transfer(this.wasabiWETH.address, '10000000', { from: minter });
        await this.weth.transfer(this.wasabiWETH.address, '10000000', { from: minter });
        await this.wasabiWETH.mint(minter);
        assert.equal((await this.wasabiWETH.balanceOf(this.maker.address)).valueOf(), '16537');
        await this.maker.convert(this.wasabi.address, this.weth.address);
        assert.equal((await this.wasabi.balanceOf(bar)).valueOf(), '66249');
        assert.equal((await this.wasabiWETH.balanceOf(this.maker.address)).valueOf(), '0');
    });
});